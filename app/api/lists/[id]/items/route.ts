...
export async function GET(request, { params }) {
  try {
    ...
    const list = db.prepare('SELECT user_id FROM lists WHERE id = ?').get(id) as { user_id: string } | undefined;
    if (!list) {
...
export async function POST(request, { params }) {
  try {
    ...
    const listCheck = db.prepare('SELECT id, user_id FROM lists WHERE id = ?').get(id) as { id: number, user_id: string } | undefined;
    if (!listCheck) {
      return NextResponse.json({ error: 'List not found' }, { status: 404 });
    }
    ...
    const existing = db.prepare('SELECT id FROM list_items WHERE list_id = ? AND title_id = ?').get(id, title_id) as { id: number } | undefined;
    if (existing) {
      return NextResponse.json(
...
export async function PATCH(request, { params }) {
  try {
    ...
    const source = db.prepare('SELECT id, user_id FROM lists WHERE id = ?').get(id) as { id: number, user_id: string } | undefined;
    const target = db.prepare('SELECT id, user_id FROM lists WHERE id = ?').get(target_list_id) as { id: number, user_id: string } | undefined;
