...
const GENRES = [
  'Action', 'Comedy', 'Drama', 'Horror', 'Thriller', 'Romance', 'Sci-Fi', 'Fantasy', 'Adventure', 'Biography'
];
...
export default function TopRated() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      setLoading(true);
      try {
        const typeToFetch = activeTab === 'movies' ? 'MOVIE' : 'TV_SERIES';
        const params = new URLSearchParams();
        
        params.append('types', typeToFetch);
        params.append('sortBy', 'SORT_BY_USER_RATING');
        params.append('sortOrder', 'DESC');
        params.append('minVoteCount', '25000');
        ...
        if (filters.country) {
          params.append('countryCodes', filters.country);
        }
