...
import { getTitleDetails, getTitleEpisodes, saveRating, getRating, getLists, addListItem, removeListItem, getTitleCredits, getTitleImages, getTitleAwards, getTitleBoxOffice, getTitleLogo, getTitleSeasons } from '@/lib/api';
import { motion, AnimatePresence } from 'framer-motion';
import { Star, Clock, Calendar, ArrowLeft, Play, MagnifyingGlass, CaretDown, Monitor, FilmReel, UserCircle, Trophy, CurrencyDollar, PlayCircle, X } from '@phosphor-icons/react';
...
export default function TitleDetails({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
    ...
    const fetchData = async () => {
      try {
        ...
        if (listsData.status === 'fulfilled') {
          ...
          // Find if the title is in any list
          const activeList = listsData.value.find(l => l.has_title);
          if (activeList) {
             setActiveStatus(activeList.name);
          }
        }
  ...
  const handleStatusChange = async (statusListName) => {
    ...
    if (!targetList) {
      console.error('List not found:', statusListName);
      return;
    }

    // Check if item is already in a different list
    const currentList = lists.find(l => l.has_title);
    
    try {
      // If already in a list, remove from it first
      if (currentList && currentList.id !== targetList.id) {
        await removeListItem(currentList.id, id);
      }
      
      // Add to new list
      const result = await addListItem(targetList.id, id);
      if (result.alreadyExists && currentList?.id === targetList.id) {
        // Already in this list, no UI update needed
        return;
      }
      
      // Update UI
      setActiveStatus(statusListName);
      // Refresh lists to update has_title status
      const updatedLists = await getLists(id);
    ...
    <div className="min-h-screen text-white selection:bg-white selection:text-black">
      ...
      <div className="max-w-[77rem] mx-auto pt-8 mt-0 pb-20">
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          <div className="w-full lg:w-[280px] flex-shrink-0 relative lg:sticky lg:top-8 self-start">
                ...
                <div className="flex flex-wrap items-center gap-3 border-b border-white/5 pb-4">
                  ...
                  {activeStatus && (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 hover:border-red-500/50 text-red-400 hover:text-red-300 text-sm font-medium transition-all duration-200"
                    >
                      <X size={16} />
                      <span>Remove</span>
                    </button>
                  ...
                  <div className="flex items-center gap-2 ml-auto">
                    ...
                    <RatingSlider 
                      ...
                      onChange={(e) => { 
                        ...
                        setUserRating(score); 
                        saveRating(id, score, '').catch(console.error); 
                      }} 
                    />
                    {userRating > 0 && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-24">
                      {episodes.map((ep) => (
                        <Link
                          ...
                          ref={(el) => { if (el) episodeRefs.current[ep.episodeNumber] = el; }}
                          className="block"
                        >
                          <motion.div
                            whileTap={{ scale: 0.98 }}
