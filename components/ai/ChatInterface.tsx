...
function parseMediaSegments(content: string) {
  if (!content) return [];
...
const processTextContent = (text: any) => {
...
const MarkdownComponents: any = {
  p: ({ children }: any) => <p className="mb-5 last:mb-0 leading-8 text-white text-base" style={{ fontSize: '17.6px', lineHeight: '1.8' }}><RichText>{children}</RichText></p>,
  strong: ({ children }: any) => <strong className="font-bold text-white tracking-wide"><RichText>{children}</RichText></strong>,
  em: ({ children }: any) => <em className="italic text-zinc-300"><RichText>{children}</RichText></em>,
  h1: ({ children }: any) => <h1 className="text-3xl font-bold mb-5 mt-9 first:mt-0 text-white tracking-tight" style={{ fontSize: '33px' }}><RichText>{children}</RichText></h1>,
  h2: ({ children }: any) => <h2 className="text-2xl font-bold mb-4 mt-7 first:mt-0 text-white tracking-tight border-b border-zinc-800 pb-3" style={{ fontSize: '24px' }}><RichText>{children}</RichText></h2>,
  h3: ({ children }: any) => <h3 className="text-xl font-bold mb-3 mt-5 first:mt-0 text-white" style={{ fontSize: '19.8px' }}><RichText>{children}</RichText></h3>,
  ul: ({ children }: any) => <ul className="list-disc ml-6 mb-5 space-y-2 text-white marker:text-zinc-500">{children}</ul>,
  ol: ({ children }: any) => <ol className="list-decimal ml-6 mb-5 space-y-2 text-white marker:text-zinc-500">{children}</ol>,
  li: ({ children }: any) => <li className="leading-8 pl-1 text-base" style={{ fontSize: '17.6px' }}><RichText>{children}</RichText></li>,
  a: ({ href, children }: any) => (
  ...
  table: ({ children }: any) => <div className="overflow-x-auto mb-5"><table className="border-collapse w-full">{children}</table></div>,
  th: ({ children }: any) => <th className="border border-zinc-700 bg-zinc-800/50 p-3 text-left font-semibold text-white"><RichText>{children}</RichText></th>,
  td: ({ children }: any) => <td className="border border-zinc-700 p-3 text-white"><RichText>{children}</RichText></td>,
};
...
const ThinkingMarkdownComponents: any = {
  p: ({ children }: any) => <p className="mb-2 last:mb-0 leading-7 text-zinc-500 text-sm">{children}</p>,
  strong: ({ children }: any) => <strong className="font-semibold text-zinc-400">{children}</strong>,
  em: ({ children }: any) => <em className="italic text-zinc-500">{children}</em>,
  h1: ({ children }: any) => <h1 className="text-lg font-bold mb-2 mt-4 text-zinc-400">{children}</h1>,
  h2: ({ children }: any) => <h2 className="text-base font-bold mb-2 mt-3 text-zinc-400">{children}</h2>,
  h3: ({ children }: any) => <h3 className="text-sm font-bold mb-1 mt-2 text-zinc-400">{children}</h3>,
  ul: ({ children }: any) => <ul className="list-disc ml-4 mb-2 space-y-1 text-zinc-500 marker:text-zinc-600">{children}</ul>,
  ol: ({ children }: any) => <ol className="list-decimal ml-4 mb-2 space-y-1 text-zinc-500 marker:text-zinc-600">{children}</ol>,
  li: ({ children }: any) => <li className="leading-6 pl-1 text-sm">{children}</li>,
  a: ({ href, children }: any) => <a href={href} className="text-zinc-400 hover:text-zinc-300 underline underline-offset-2" target="_blank" rel="noreferrer">{children}</a>,
  code: ({ inline, className, children }: any) => {
  ...
  blockquote: ({ children }: any) => <blockquote className="border-l-2 border-zinc-700 pl-3 italic text-zinc-500 my-2">{children}</blockquote>,
};

const ThinkingMarkdown = memo(({ content }: { content: string }) => (
  <ReactMarkdown
...
const ThinkingExpandable = ({ content, isFinished }: { content: string, isFinished: boolean }) => {
...
export default function ChatInterface({ 
  ...
  const [toolProgress, setToolProgress] = useState<any>(null);
  const [isSearchingWeb, setIsSearchingWeb] = useState(false);
  const [expandSources, setExpandSources] = useState(false);
  const [hasToken, setHasToken] = useState<boolean | null>(null);
  const [activeStream, setActiveStream] = useState<any>(null);
  const [sessionActivity, setSessionActivity] = useState<{ searches: any[], toolCalls: any[], listEdits: any[] }>({
    searches: [],
    toolCalls: [],
    listEdits: []
  });
  ...
  useEffect(() => {
    fetch('/api/ai/models')
      ...
      .then((data) => {
        ...
        setModels(list);
        if (list.length > 0) {
          const preferredModel = list.find((m: any) => m.id === 'gpt-4.1') ||
            list.find((m: any) => m.id.includes('gpt-4.1')) ||
            list.find((m: any) => m.id.includes('gpt-4-turbo')) ||
            list.find((m: any) => m.id.includes('gpt-4')) ||
            list[0];
  ...
  const stripMediaGrid = (content: string) => {
    if (!content) return '';
    let cleaned = content;
  ...
  const extractThinking = (text: string, thinkingStarted?: number): { thinking: string | null, content: string, isFinished: boolean } => {
        ...
        for (const part of parts) {
          ...
          try {
            ...
            if (data.type === 'chat_created') {
              ...
              const titles = (data.titles || []).map(normalizeTitle).filter((t: any) => t?.id);
              setMessages((prev) => prev.map((msg) => {
                  if (msg.id !== aiMsgId) return msg;
                  const existingRecs = msg.recommendations || [];
                  const existingIds = new Set(existingRecs.map((r: any) => r.id));
                  const newTitles = titles.filter((t: any) => !existingIds.has(t.id));
              ...
              if (sources.length > 0) {
                ...
                setMessages((prev) => prev.map((msg) => {
                  ...
                  const existingUrls = new Set(existing.map((src: any) => src.url));
              ...
              if (data.tool === 'bulk_add_to_list') {
                setToolProgress((prev: any) => prev ? ({ ...prev, status: 'done', completed: prev.total || prev.completed }) : null);
              ...
              if (data.tool === 'web_search') {
                ...
                setMessages((prev) => prev.map((msg) => {
                    if (msg.id !== aiMsgId) return msg;
                    const existingRecs = msg.recommendations || [];
                    if (existingRecs.find((r: any) => r.id === normalized.id)) return msg;
      ...
      <div className="flex-1 flex flex-col h-full relative min-w-0 bg-transparent overflow-hidden">
                            ...
                            <>
                              ...
                              {msg.streamCard && (
                                <div className="mb-6">
                                   <InlineStreamCard
                                      data={msg.streamCard}
                                      onPlay={(data: any) => setActiveStream({
                                        imdbId: data.imdb_id,
                                        season: data.season,
                                ...
                                <div className="mb-6">
                                  ...
                                  <AnimatePresence>
                                    {expandSources && msg.sources.length > 4 && (
                                      ...
                                      >
                                        {msg.sources.slice(4).map((source: any, index: number) => {
                                          const favicon = getFaviconUrl(source.url);
                                          return favicon ? (
                                            <a
        ...
        <AnimatePresence>
          {activeStream && (
             <StreamPlayer
                title={{
                  id: activeStream.imdbId,
                  primaryTitle: activeStream.title,
                  type: activeStream.type
                }}
                initialSeason={activeStream.season}
                initialEpisode={activeStream.episode}
