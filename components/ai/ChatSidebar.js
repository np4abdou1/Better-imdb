...
export default function ChatSidebar({ chats: initialChats }) {
  ...
  const [activeId, setActiveId] = useState(activeChatId || null);
  ...
  useEffect(() => {
    ...
    window.addEventListener('orb-chat-created', handleChatCreated);
    window.addEventListener('orb-title-updated', handleTitleUpdated);

    return () => {
      window.removeEventListener('orb-chat-created', handleChatCreated);
      window.removeEventListener('orb-title-updated', handleTitleUpdated);
    };
  }, []);
