...
import { Film, Star, Plus, Check, Eye, Clock, Bookmark, List } from 'lucide-react';
 ...
 * Props:
 *   - id: IMDB ID (tt...)
 *   - title: Title string (fallback)
 *   - year: Year number (fallback)
 *   - reason: Recommendation reason string
 *   - index: Optional index for stagger animation
 */
function InlineMediaCard({ id, title, year, reason, index = 0 }) {
  ...
  const handleAddToList = async (e, listName) => {
    e.preventDefault();
    e.stopPropagation();
          ...
          <div className="flex items-center gap-3 text-xs font-medium text-zinc-300">
            ...
            {data.type && (
              <span className="px-2 py-0.5 bg-white/10 rounded text-[10px] uppercase border border-white/10">
                {data.type === 'tvMiniSeries' || data.type === 'TV_MINI_SERIES'
                  ? 'Mini'
                  : data.type === 'tvSeries' || data.type === 'TV_SERIES'
                    ? 'TV'
                    : 'Movie'}
              </span>
            )}
