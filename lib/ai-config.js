 ...
 * @param {string|null} userId - User ID to fetch data for (can be null for anonymous)
 * @returns {Promise<string>} Natural language taste profile
 */
export async function generateTasteProfile(userId) {
  ...
  if (ratings && ratings.length > 0) {
    const highRated = ratings.filter(r => r.score >= 8);
    const lowRated = ratings.filter(r => r.score <= 4);
    ...
    if (recent5.length > 0) {
      const recentList = recent5.map(r => `${r.title_id} (${r.score}/10)`).join(', ');
  ...
  if (lists.length > 0) {
    ...
    const toWatchList = lists.find(l => l.name === 'To Watch');
    const watchingList = lists.find(l => l.name === 'Watching');
    const favoritesList = lists.find(l => l.name === 'Favorites');
...
**Example structure:**
  ...
  [inline media_grid block 1]

  If you prefer something darker: 
  
  [inline media_grid block 2]"

INLINE CARD SYNTAX:
Place media_grid blocks naturally within your response text (not necessarily at the end).
Use multiple smaller media_grid blocks (1-3 cards each) for better flow instead of one large block.
...
When recommending, spread cards throughout the response where they're contextually relevant.
...
- Use multiple smaller media_grid blocks instead of one large one for better inline rendering
...
export const TOOL_DEFINITIONS = [
  {
    ...
    description: 'PREFERRED: Search and resolve multiple movie/TV titles in parallel. Use this when recommending multiple titles. Supports large batches (up to 100). Returns resolved IMDB IDs and full metadata for all queries.',
    input_schema: {
      type: 'object',
      properties: {
        queries: {
...
export function getToolsForCopilot() {
